...
export const Player: React.FC = () => {
  ...
  // --- Controls (Keyboard & Touch & Virtual) ---
  ...
  useEffect(() => {
    ...
    const handleVirtualAbility = () => {
        if (status !== GameStatus.PLAYING) return;
        activateImmortality();
    };

    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('virtual-move-left', handleVirtualLeft);
    window.addEventListener('virtual-move-right', handleVirtualRight);
    window.addEventListener('virtual-jump', handleVirtualJump);
    window.addEventListener('virtual-ability', handleVirtualAbility);

    return () => {
        window.removeEventListener('keydown', handleKeyDown);
        window.removeEventListener('virtual-move-left', handleVirtualLeft);
        window.removeEventListener('virtual-move-right', handleVirtualRight);
        window.removeEventListener('virtual-jump', handleVirtualJump);
        window.removeEventListener('virtual-ability', handleVirtualAbility);
    };
  }, [status, laneCount, triggerJump, activateImmortality]);
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        // Swipe Detection (only if significant movement, to allow tapping virtual buttons without triggering swipe)
        // Increased threshold slightly to prevent accidental swipes when using buttons if they overlap (though buttons preventDefault)
        ...
        // Removed tap-to-ability on center because it conflicts with UI interactions often.
        // Virtual button handles ability now.
    };

    // Only add touch listeners to the canvas container or window. 
    // Since this component is inside Canvas, we attach to window but we should be careful about UI.
    // However, the UI buttons call stopPropagation, so this should be fine.
